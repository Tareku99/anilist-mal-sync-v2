...
def main():
    ...
    try:
        ...
        logger.error(f"❌ UNHEALTHY: Failed to load configuration: {e}")
        sys.exit(1)
    
    # Check if token file exists
    token_file = Path(settings.token_file)
    if not token_file.exists():
        logger.error("❌ UNHEALTHY: Token file not found")
    ...
    try:
        ...
        if not tokens_data or "tokens" not in tokens_data:
            logger.error("❌ UNHEALTHY: No tokens found in token file")
            logger.error(AUTH_INSTRUCTION)
            sys.exit(1)
        ...
        if not tokens.get("anilist", {}).get("access_token"):
            logger.error("❌ UNHEALTHY: AniList access token missing")
            logger.error(AUTH_INSTRUCTION)
            sys.exit(1)
        
        if not tokens.get("mal", {}).get("access_token"):
            logger.error("❌ UNHEALTHY: MAL access token missing")
            logger.error(AUTH_INSTRUCTION)
            sys.exit(1)
        
        logger.info("✅ HEALTHY: All tokens present")
        sys.exit(0)
        
    except Exception as e:
        logger.error(f"❌ UNHEALTHY: Token validation failed: {e}")
        sys.exit(1)
